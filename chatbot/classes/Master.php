...
class Master extends DBConnection
{
	...
	public function get_response()
	{
		...
		if ($qry->num_rows > 0) {
			...
			// Check if the response message is a hyperlink
			if (filter_var($result['response_message'], FILTER_VALIDATE_URL)) {
				// If the response message is a hyperlink, add HTML anchor tags around it
				$resp['status'] = "success";
				$resp['message'] = "<a href='" . $result['response_message'] . "'>" . $result['response_message'] . "</a>";
			} else {
				// If the response message is not a hyperlink, just return it as normal
				$resp['status'] = "success";
